...
import {MD5} from "../../services/md5/md5";
const blogName = "senierp.wordpress.com";
...
export function getDoc(slug, password?) {
  ...
  return function(dispatch) {
    ...
    fetch(`https://public-api.wordpress.com/rest/v1.1/sites/${blogName}/posts/slug:${slug}`)
      ...
      .then((json) => {
        if (password !== undefined) {
          ...
          fetch(`https://public-api.wordpress.com/rest/v1.2/sites/${blogName}`)
            .then((response) => response.json())
            .then((pass) => {
...
export function searchDoc(query) {
  ...
  return function(dispatch) {
    ...
    if (query === "") {
      ...
      fetch(`https://public-api.wordpress.com/rest/v1.1/sites/${blogName}/posts/?search=${query}&number=10&fields=ID,title,slug,excerpt`)
        .then((response) => response.json())
        .then((json) => {
